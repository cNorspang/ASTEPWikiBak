...
- `Creator`:
  ...
  The Creator contains the definition of the `data_model.py`(TNM) , and an `adaptor.py` file that handles the conversion from database rows to the TNM. Whenever new data is introduced to any microservice it is important to update the data_model.py to include node, edge and connversionScheme definitions that can handle the new data see [data_model](#data_model_py)  
- `Trimmer`:
...
- The internal representation may be as different and chaotic as necessary, but when serialized into a valid schema, all restrictions presented in [The Format](#the-format) and [0020](/rfc/0020#combination-of-weights)[Combination of Weights](#combination-of-weights) must be upheld.
An example of a conversionScheme function is shown below in the 
  ### Data
  ``` python
    """
