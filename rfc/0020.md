...
Too add a new weighter or router service to the transportation pipeline the controller microservice has to be accessed. This is done thorugh the [GitLab](https://daisy-git.cs.aau.dk/astep-2021/group-11/tnm-controller) of the controller microservice. At the top of the "service.py" file two lists are maintained of all the weighter and router services currently accesable for the controller. To integrate a new service add the name and URL of said service to either the weighter or router list depending on the service. After this has been done and pushed onto the master branch where it hopefully passes the pipeline, you should be able to see your newly added service in the dropdown menues of the TNM service on aSTEP.
Below a code snippet with the two mentioned lists can be seen.
  
TODO ADD BILLEDE AF LISTER NÅR CONTROLLER ER DONE
