...
- The commitee meeting will start on time, and anyone late cannot be expected to get a refresher of what has happened until then.
...
- Then a "Walk the board" will be done, where all current issues will be gone through the [Trello Board](https://trello.com/b/pp5iJXGi/astep) as to if they have advanced, if they are in progress or if they are blocked in some way. This should take at max 10 minutes.
...
- SPECIAL CASE: If 100% of the present Scrum Masters of a given super group (Time Series or Transportation) wants to discard a proposal, it will be discarded, not withstanding if it follows the above criteria.
...
The current sprints (2021), with a summary of the Sprint Review when the sprint is finished, are:
